...
LOG_MODULE_REGISTER(crypto_smartbond_crypto);
...
#if defined(CONFIG_CRYPTO_ASCYN)
...
/* The crypto engine consist of two sub-blocks: AES and HASH. Only one sub-block can operate at a time. */
static struct crypto_smartbond_session crypto_smartbond_sessions[CRYPTO_MAX_SESSION];
...
static int crypto_smartbond_get_unused_session_index(const struct device *dev)
{
    int idx = -1;
    ...
    for (int i = 0; i < CRYPTO_MAX_SESSION; i++) {
        if (!crypto_smartbond_sessions[i].in_use) {
            crypto_smartbond_sessions[i].in_use = true;
            idx = i;
        }
    }
...
{
    uint32_t not_last_in_block = AES_HASH->CRYPTO_CTRL_REG & AES_HASH_CRYPTO_CTRL_REG_CRYPTO_MORE_IN_Msk;
    ...
    if (AES_HASH->CRYPTO_CTRL_REG & AES_HASH_CRYPTO_CTRL_REG_CRYPTO_HASH_SEL_Msk) {
        ...
        if (in_len & 0xF) {
            ...
            uint32_t crypto_mode = (AES_HASH->CRYPTO_CTRL_REG & AES_HASH_CRYPTO_CTRL_REG_CRYPTO_ALG_MD_Msk) >>
                                                                AES_HASH_CRYPTO_CTRL_REG_CRYPTO_ALG_MD_Pos;
            /* Check if AES mode is ECB */
            if (crypto_mode == 0x0 || crypto_mode == 0x1) {
                return -EINVAL;
            }
        }
    }
...
#define SET_HASH_OUT_LEN(_len)  AES_HASH->CRYPTO_CTRL_REG = (AES_HASH->CRYPTO_CTRL_REG & \
                                                     ~(AES_HASH_CRYPTO_CTRL_REG_CRYPTO_HASH_OUT_LEN_Msk)) | \
                                                      ((_len) << AES_HASH_CRYPTO_CTRL_REG_CRYPTO_HASH_OUT_LEN_Pos)
...
static int crypto_smartbond_hash_set_out_len(void)
{
    ...
    return ((AES_HASH->CRYPTO_CTRL_REG & AES_HASH_CRYPTO_CTRL_REG_CRYPTO_HASH_OUT_LEN_Msk) >>
                                         AES_HASH_CRYPTO_CTRL_REG_CRYPTO_HASH_OUT_LEN_Pos) + 1;
}
...
static int crypto_smartbond_cipher_key_load(uint8_t *key, uint16_t key_len)
{
    ...
    if (key_len == 32) {
        AES_HASH->CRYPTO_CTRL_REG |= (0x2 << AES_HASH_CRYPTO_CTRL_REG_CRYPTO_AES_KEY_SZ_Pos);
    } else if (key_len == 24) {
        AES_HASH->CRYPTO_CTRL_REG |= (0x1 << AES_HASH_CRYPTO_CTRL_REG_CRYPTO_AES_KEY_SZ_Pos);
    } else if (key_len == 16) {
        /* Nothing to do */
    } else {
        return -EINVAL;
    ...
    if (IS_ADDRESS_USER_DATA_KEYS_SEGMENT((uint32_t)key)) {
        ...
        da1469x_otp_read(da1469x_otp_address_to_cell_offset((uint32_t)key),
                                                (void *)&AES_HASH->CRYPTO_KEYS_START, key_len);
    } else {
        volatile uint32_t *kmem_ptr = &AES_HASH->CRYPTO_KEYS_START;

        do {
            *(kmem_ptr++) = crypto_smartbond_swap_word(key); // TODO: Check if key is updated in subroutine *(key++)
            key += 4;
            key_len -= 4;
        } while (key_len);
    }
...
{
    /* Select HASH mode */
    AES_HASH->CRYPTO_CTRL_REG = (AES_HASH->CRYPTO_CTRL_REG & ~(AES_HASH_CRYPTO_CTRL_REG_CRYPTO_ALG_Msk |
                                                              AES_HASH_CRYPTO_CTRL_REG_CRYPTO_ALG_MD_Msk)) |
                                 AES_HASH_CRYPTO_CTRL_REG_CRYPTO_HASH_SEL_Msk |
                                 (0x1 <<AES_HASH_CRYPTO_CTRL_REG_CRYPTO_ALG_MD_Pos);
    switch (algo) {
    case CRYPTO_HASH_ALGO_SHA224:
        /* CRYPTO_ALG_MD = 0x1 defines 32-bit operations */
...
static int crypto_smartbond_set_in_out_buf(uint8_t *in_buf, uint8_t *out_buf, int len)
{
    ...
    /* Input data can reside in any address space. Cryto DMA can only access physical addresses (not remapped). */
    uint32_t phy_addr = black_orca_phy_addr((uint32_t)in_buf);

    if (IS_QSPIF_CACHED_ADDRESS(phy_addr)) {
        /*
         * To achiebe max. perfomance, peripherals should not access the Flash memory through the instruction
         * cache controller (avoid cache misses).
         */
    ...
    /*
     * OUT buffer can be NULL in case of fregmented data processing. CRYPTO_DEST_ADDR and CRYPTO_FETCH_ADDR
     * are being updated as calculations prceeds and OUT data are written into the memory.
     */
    if (out_buf) {
        uint32_t remap_adr0 = CRG_TOP->SYS_CTRL_REG & CRG_TOP_SYS_CTRL_REG_REMAP_ADR0_Msk;

        /*
         * OUT data can only be written in SYSRAM, non-cached remapped SYSRAM and cached non-remapped SYSRAM */
        if (IS_SYSRAM_ADDRESS(out_buf) || (IS_REMAPPED_ADDRESS(out_buf) && remap_adr0 == 3)) {
...
static int crypto_smartbond_cipher_store_iv(uint8_t *iv, uint32_t len_words)
{
    if (iv == NULL || len_words == 0 || len_words > 4) {
        return -EINVAL;
    }
    ...
    crypto_smartbond_cipher_store_dep_data((uint32_t *)iv, len_words);

    /* Return success */
    return 0;
}

static int crypto_smartbond_cipher_store_ic(uint8_t *ic, uint32_t len_words)
{
    if (ic == NULL || len_words == 0 || len_words > 4) {
        return -EINVAL;
    }
    ...
    crypto_smartbond_cipher_store_dep_data((uint32_t *)ic, len_words);
...
static int crypto_smartbond_cipher_ecb_handler(struct cipher_ctx *ctx, struct cipher_pkt *pkt)
{
    ...
    if (pkt->out_buf_max < pkt->in_len) {
        LOG_ERR("OUT buffer cannot be lesst that IN buffer");
        return -EINVAL;
    }
    ...
    if (pkt->in_len > 16) {
        LOG_ERR("For security reasons, ECB should not operate on more than 1 block (16 bytes)");
        return -EINVAL;
    }
...
static int crypto_smartbond_cipher_cbc_handler(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
    int ret;
    int offset = 0;
    struct crypto_smartbond_data *data = ctx->device->data;
    bool is_op_encryption = !!(AES_HASH->CRYPTO_CTRL_REG & AES_HASH_CRYPTO_CTRL_REG_CRYPTO_ENCDEC_Msk);
    ...
    ret = crypto_smartbond_cipher_store_iv(iv, 4);
    if (ret < 0) {
    ...
    if (is_op_encryption) {
        ret = crypto_smartbond_set_in_out_buf(pkt->in_buf, pkt->out_buf + offset, pkt->in_len);
    } else {
        ret = crypto_smartbond_set_in_out_buf(pkt->in_buf + offset, pkt->out_buf, pkt->in_len - offset);
    }
    if (ret < 0) {
        LOG_ERR("Unsupported IN or OUT buffer location");
        k_sem_give(&data->device_sem);
...
static int crypto_smartbond_cipher_ctr_handler(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *ic)
{
    int ret;
    ...
    ret = crypto_smartbond_cipher_store_ic(ic, iv_len >> 2);
    if (ret < 0) {
                                                                            ...
                                                                            enum cipher_op op_type)
{
    int ret;
    int session_idx;
    struct crypto_smartbond_session *session;
    ...
    session_idx = crypto_smartbond_get_unused_session_index(dev);
    if (session_idx < 0) {
        LOG_ERR("No free session for now");
        return -ENOSPC;
    }
    /* From this point onwards the device is locked. */
    session = &crypto_smartbond_sessions[session_idx];
...
{
    struct crypto_smartbond_session *session = ctx->drv_sessn_state;
    struct crypto_smartbond_data *data = dev->data;
    ...
    /* Crypto should be disabled only if not used in other sessions */
    for (int i = 0; i < CRYPTO_MAX_SESSION; i++) {
        if (crypto_smartbond_sessions[i].in_use) {
            k_sem_give(&data->session_sem);
            return 0;
...
#if defined(CONFIG_CRYPTO_ASYNC)
static int crypto_smartbond_cipher_set_async_callback(const struct device *dev, cipher_completion_cb cb)
{
    struct crypto_smartbond_data *data = dev->data;
...
static int crypto_smartbond_hash_begin_session(const struct device *dev,
...
#define CRYPTO_INST_GET_DATA(_inst)   crypto_smartbond_data_##inst
...
#if !defined(CONFIG_CRYPTO_ASYNC)
    /* Sempahore used when async operations are enabled */
    k_sem_init(&data->sync_sem, 0, 1);
#endif

    IRQ_CONNECT(IRQN, IRQ_PRIO, smartbond_crypto_isr, DEVICE_DT_INST_GET(0), 0);
...
DT_INST_FOREACH_STATUS_OKAY(CRYPTO_INIT)