...
static int dma_smartbond_config(const struct device *dev, uint32_t channel, struct dma_config *cfg)
{
	...
	if (cfg->source_handshake != cfg->dest_handshake) {
		LOG_ERR("Source and destination handshakes mismatch");
		return -EINVAL;
	}

	if (!cfg->source_handshake && (cfg->dma_slot == DMA_SMARTBOND_TRIG_MUX_NONE ||
				DMA_CTRL_REG_GET_FIELD(DREQ_MODE, dma_ctrl_reg) != DREQ_MODE_HW)) {
		LOG_ERR("HW handshake is selected but no triggering source is selected");
		return -EINVAL;
	}
