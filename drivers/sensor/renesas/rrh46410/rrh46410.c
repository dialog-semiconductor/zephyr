...
static uint8_t rrh46410_get_operation_mode(const struct device *dev)
{
	...
	a[0] = RRH46410_GET_OPERATION_MODE;
	a[1] = checksum;
	rc =  data->hw_tf->write_data(dev, a, sizeof(a));
...
static int rrh46410_read_sample(const struct device *dev, uint8_t *sample_counter, uint8_t *iaq_sample, uint16_t *tvoc_sample, uint16_t *etoh_sample, uint16_t *eco2_sample, uint8_t *reliaq_sample)
{
	struct rrh46410_data *data = dev->data;
	//1 status, 1 counter, 8 data bytes, 1 checksum
	uint8_t status;
	//uint8_t checksum;
	//uint8_t checksum_sensor;
	int rc;
	...
	if( *sample_counter == data->read_buffer[1]){
		LOG_ERR("Stale data");
		//return 0;
	}
	//remove later
	else{
		LOG_ERR("Sample counter: %d", *sample_counter);
	}
...
static int rrh46410_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	a[0] = RRH46410_GET_MEASUREMENT_RESULTS;
	a[1] = checksum;
	...
	rc = rrh46410_read_sample(dev, &data->sample_counter, &data->iaq_sample, &data->tvoc_sample, &data->etoh_sample, &data->eco2_sample, &data->reliaq_sample);
	if (rc < 0) {
...
static int rrh46410_init(const struct device *dev)
{
	...
	if (rrh46410_mode != 0x01){
		LOG_ERR("Sensor is in %u mode", rrh46410_mode);
		rrh46410_set_operation_mode(dev);
	}else{
		LOG_ERR("Sensor already in %u mode", rrh46410_mode);
	}
...
static const struct sensor_driver_api rrh46410_driver_api = {.sample_fetch = rrh46410_sample_fetch,
