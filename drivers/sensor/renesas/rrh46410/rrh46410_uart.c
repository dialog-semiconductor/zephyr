...
{
	memset(data->uart_buffer, 0, data->read_index);
	data->read_index = 0;
}
...
static void rrh46410_uart_isr(const struct device *uart_dev, void *user_data)
{
	...
	rc = uart_fifo_read(uart_dev, &data->uart_buffer[data->read_index], RRH46410_BUFFER_LENGTH);

	if (rc < 0) {
		LOG_ERR("UART read failed: %d", rc < 0 ? rc : -ERANGE);
		rrh46410_uart_flush(uart_dev);
		LOG_HEXDUMP_WRN(data->uart_buffer, data->read_index, "Discarding");
		rrh46410_buffer_reset(data);
	} else {
		data->read_index += rc;

		for (int i = 1; i < data->read_index - 1; i++) {
			received_checksum += data->uart_buffer[i];
		}

		received_checksum = ~received_checksum;

		if (data->uart_buffer[data->read_index - 1] == received_checksum) {
...
static int rrh46410_uart_transceive(const struct device *dev, uint8_t *command_data,
				    size_t data_size)
{
...
static int rrh46410_uart_read_data(const struct device *dev, uint8_t *rx_buff, size_t data_size)
{
	...
	memcpy(data->read_buffer, data->uart_buffer, sizeof(data->read_buffer));
