...
#define DMA_CHANNEL_RX                 DT_PROP(DT_NODELABEL(usbd), dma_chan_rx)
#define DMA_CHANNEL_TX                 DT_PROP(DT_NODELABEL(usbd), dma_chan_tx)
 ...
 */
#define DMA_MIN_TRANSFER_SIZE          DT_PROP(DT_NODELABEL(usbd), dma_min_transfer_size)
#define FIFO_READ_THRESHOLD            DT_PROP(DT_NODELABEL(usbd), fifo_read_threshold)
...
#define EP1_OUT_BUF_SIZE               DT_PROP_BY_IDX(DT_NODELABEL(usbd), ep_out_buf_size, 1)
#define EP2_OUT_BUF_SIZE               DT_PROP_BY_IDX(DT_NODELABEL(usbd), ep_out_buf_size, 2)
#define EP3_OUT_BUF_SIZE               DT_PROP_BY_IDX(DT_NODELABEL(usbd), ep_out_buf_size, 3)
...
struct usb_ep_reg_set {
	union {
		__IOM uint32_t epc_in;
		__IOM uint32_t USB_EPC0_REG;
		__IOM uint32_t USB_EPC1_REG;
		__IOM uint32_t USB_EPC3_REG;
		__IOM uint32_t USB_EPC5_REG;
	};
	union {
		__IOM uint32_t txd;
		__IOM uint32_t USB_TXD0_REG;
		__IOM uint32_t USB_TXD1_REG;
		__IOM uint32_t USB_TXD2_REG;
		__IOM uint32_t USB_TXD3_REG;
	};
	union {
		__IOM uint32_t txs;
		__IOM uint32_t USB_TXS0_REG;
		__IOM uint32_t USB_TXS1_REG;
		__IOM uint32_t USB_TXS2_REG;
		__IOM uint32_t USB_TXS3_REG;
	};
	union {
		__IOM uint32_t txc;
		__IOM uint32_t USB_TXC0_REG;
		__IOM uint32_t USB_TXC1_REG;
		__IOM uint32_t USB_TXC2_REG;
		__IOM uint32_t USB_TXC3_REG;
	};
	union {
		__IOM uint32_t epc_out;
		__IOM uint32_t USB_EP0_NAK_REG;
		__IOM uint32_t USB_EPC2_REG;
		__IOM uint32_t USB_EPC4_REG;
		__IOM uint32_t USB_EPC6_REG;
	};
	union {
		__IOM uint32_t rxd;
		__IOM uint32_t USB_RXD0_REG;
		__IOM uint32_t USB_RXD1_REG;
		__IOM uint32_t USB_RXD2_REG;
		__IOM uint32_t USB_RXD3_REG;
	};
	union {
		__IOM uint32_t rxs;
		__IOM uint32_t USB_RXS0_REG;
		__IOM uint32_t USB_RXS1_REG;
		__IOM uint32_t USB_RXS2_REG;
		__IOM uint32_t USB_RXS3_REG;
	};
	union {
		__IOM uint32_t rxc;
		__IOM uint32_t USB_RXC0_REG;
		__IOM uint32_t USB_RXC1_REG;
		__IOM uint32_t USB_RXC2_REG;
		__IOM uint32_t USB_RXC3_REG;
	};
};

static struct usb_ep_reg_set *const reg_sets[4] = {
	(struct usb_ep_reg_set *)&USB->USB_EPC0_REG,
	(struct usb_ep_reg_set *)&USB->USB_EPC1_REG,
	(struct usb_ep_reg_set *)&USB->USB_EPC3_REG,
	(struct usb_ep_reg_set *)&USB->USB_EPC5_REG,
};
...
struct usb_dc_state {
	...
	struct usb_ep_state ep_state[2][4];
	atomic_ptr_t dma_ep[2];         /** DMA used by channel */
};

static struct usb_dc_state usb_dc_state;
...
static struct usb_ep_state *usb_dc_get_ep_state(uint8_t ep)
{
	uint8_t ep_num = USB_EP_GET_IDX(ep);
	uint8_t ep_dir = USB_EP_GET_DIR(ep) ? 1 : 0;

	return &usb_dc_state.ep_state[ep_dir][ep_num];
}

static struct usb_ep_state *usb_dc_get_ep_out_state(uint8_t ep)
{
	uint8_t ep_num = USB_EP_GET_IDX(ep);

	return ep_num < EP_MAX || USB_EP_DIR_IS_OUT(ep) ?
		&usb_dc_state.ep_state[0][ep_num] : NULL;
}

static struct usb_ep_state *usb_dc_get_ep_in_state(uint8_t ep)
{
	uint8_t ep_num = USB_EP_GET_IDX(ep);

	return ep_num < EP_MAX || USB_EP_DIR_IS_IN(ep) ?
	       &usb_dc_state.ep_state[1][ep_num] : NULL;
}

static inline bool dev_attached(void)
{
	return usb_dc_state.attached;
}

static inline bool dev_ready(void)
{
	return usb_dc_state.vbus_present;
}

static void set_nfsr(uint8_t val)
{
	usb_dc_state.nfsr = val;
	/*
...
static void fill_tx_fifo(struct usb_ep_state *ep_state)
{
	int left_to_send;
	const uint8_t *src;
	uint8_t ep_num = USB_EP_GET_IDX(ep_state->ep_addr);
	struct usb_ep_reg_set *regs = ep_state->regs;

	src = &ep_state->buffer[ep_state->transferred];
	left_to_send = ep_state->total_len - ep_state->transferred;
	if (left_to_send > ep_state->max_packet_size - ep_state->last_packet_size) {
		left_to_send = ep_state->max_packet_size - ep_state->last_packet_size;
	}
	...
	if (ep_num != 0) {
		if (left_to_send > 0) {
			...
			USB->USB_FWMSK_REG |=
				1 << (ep_num - 1 + USB_USB_FWMSK_REG_USB_M_TXWARN31_Pos);
		} else {
...
static bool try_allocate_dma(struct usb_ep_state *ep_state, uint8_t dir)
{
	uint8_t ep_num = USB_EP_GET_IDX(ep_state->ep_addr);
	uint8_t dir_ix = dir == USB_EP_DIR_OUT ? 0 : 1;

	if (atomic_ptr_cas(&usb_dc_state.dma_ep[dir_ix], NULL, ep_state)) {
		if (dir == USB_EP_DIR_OUT) {
			USB->USB_DMA_CTRL_REG =
				(USB->USB_DMA_CTRL_REG & ~USB_USB_DMA_CTRL_REG_USB_DMA_RX_Msk) |
				((ep_num - 1) << USB_USB_DMA_CTRL_REG_USB_DMA_RX_Pos);
		} else {
			USB->USB_DMA_CTRL_REG =
				(USB->USB_DMA_CTRL_REG & ~USB_USB_DMA_CTRL_REG_USB_DMA_TX_Msk) |
				((ep_num - 1) << USB_USB_DMA_CTRL_REG_USB_DMA_TX_Pos);
		}
...
static void start_rx_packet(struct usb_ep_state *ep_state)
{
	uint8_t ep_num = USB_EP_GET_IDX(ep_state->ep_addr);
	struct usb_ep_reg_set *regs = ep_state->regs;
	...
	if (ep_state->max_packet_size > DMA_MIN_TRANSFER_SIZE) {
		if (try_allocate_dma(ep_state, USB_EP_DIR_OUT)) {
			start_rx_dma(&regs->rxd,
				     ep_state->buffer,
				     ep_state->max_packet_size);
		} else if (ep_state->max_packet_size > EP_FIFO_SIZE) {
			/*
			...
			USB->USB_FWMSK_REG |=
				1 << (ep_num - 1 + USB_USB_FWMSK_REG_USB_M_RXWARN31_Pos);
		}
	} else if (ep_num != 0) {
		 ...
		 */
		regs->rxc &= ~USB_USB_RXC1_REG_USB_RFWL_Msk;
		USB->USB_FWMSK_REG &= ~(1 << (ep_num - 1 + USB_USB_FWMSK_REG_USB_M_RXWARN31_Pos));
	}
...
static void start_tx_packet(struct usb_ep_state *ep_state)
{
	struct usb_ep_reg_set *regs = ep_state->regs;
	uint16_t remaining = ep_state->total_len - ep_state->transferred;
	uint16_t size = MIN(remaining, ep_state->max_packet_size);
...
static uint16_t read_rx_fifo(struct usb_ep_state *ep_state,
			     uint16_t bytes_in_fifo)
{
	struct usb_ep_reg_set *regs = ep_state->regs;
	uint16_t remaining = ep_state->max_packet_size - ep_state->last_packet_size;
...
static void handle_ep0_rx(void)
{
	...
	struct usb_ep_state *ep0_out_state = usb_dc_get_ep_out_state(0);
	struct usb_ep_state *ep0_in_state;
...
{
	uint32_t txs0;
	struct usb_ep_state *ep0_in_state = usb_dc_get_ep_in_state(0);
	struct usb_ep_state *ep0_out_state = usb_dc_get_ep_out_state(0);
	struct usb_ep_reg_set *regs = ep0_in_state->regs;
	...
	if (GET_BIT(txs0, USB_USB_TXS0_REG_USB_TX_DONE)) {
		...
		if (GET_BIT(txs0, USB_USB_TXS0_REG_USB_ACK_STAT)) {
			...
			if (ep0_in_state->transferred == ep0_in_state->total_len) {
				...
				atomic_clear(&ep0_in_state->transfer_in_progress);
...
static void handle_epx_rx_ev(uint8_t ep_num)
{
	uint32_t rxs;
	int fifo_bytes;
	struct usb_ep_state *ep_state = usb_dc_get_ep_out_state(ep_num);
	struct usb_ep_reg_set *regs = ep_state->regs;
	...
	do {
		...
		if (GET_BIT(rxs, USB_USB_RXS1_REG_USB_RX_ERR)) {
			...
			ep_state->last_packet_size = 0;
			if (usb_dc_state.dma_ep[0] == ep_state) {
				/* Stop DMA */
		...
		if (usb_dc_state.dma_ep[0] == ep_state) {
			 ...
			 */
			RX_DMA_REGS->DMAx_CTRL_REG &= ~DMA_DMA0_CTRL_REG_DMA_ON_Msk;
			ep_state->last_packet_size = RX_DMA_REGS->DMAx_IDX_REG;
			/*
			 * When DMA did not finished (packet was smaller then MPS),
			 * DMAx_IDX_REG holds exact number of bytes transmitted. When DMA
			 * finished value in DMAx_IDX_REG is one less then actual number of
			 * transmitted bytes.
			 */
			if (ep_state->last_packet_size == RX_DMA_REGS->DMAx_LEN_REG) {
				ep_state->last_packet_size++;
			}
			/* Release DMA to use by other endpoints. */
			usb_dc_state.dma_ep[0] = NULL;
		}
		...
		if (GET_BIT(rxs, USB_USB_RXS1_REG_USB_RX_LAST)) {
			if (!ep_state->iso &&
				...
				ep_state->data1 ^= 1;
				atomic_clear(&ep_state->transfer_in_progress);
...
static void handle_epx_tx_ev(struct usb_ep_state *ep_state)
{
	uint32_t txs;
	struct usb_ep_reg_set *regs = ep_state->regs;
...
{
	if (usb_dc_state.nfsr == NFSR_NODE_RESET) {
		...
		if (GET_BIT(alt_ev, USB_USB_ALTEV_REG_USB_RESET) == 0) {
			...
			if (usb_dc_state.ep_state[0][0].buffer != NULL) {
				USB->USB_MAMSK_REG |=
...
static void handle_bus_reset(void)
{
	...
	for (int i = 0; i < EP_MAX; ++i) {
		usb_dc_state.ep_state[1][i].buffer = NULL;
		usb_dc_state.ep_state[1][i].transferred = 0;
		usb_dc_state.ep_state[1][i].total_len = 0;
		atomic_clear(&usb_dc_state.ep_state[1][i].transfer_in_progress);
	}
...
static void handle_alt_ev(void)
{
	...
	if (GET_BIT(alt_ev, USB_USB_ALTEV_REG_USB_RESET) &&
		...
		if (USB->USB_NFSR_REG == NFSR_NODE_SUSPEND) {
			...
			for (int ep_num = 1; ep_num <= 3; ++ep_num) {
...
static void handle_epx_tx_warn_ev(uint8_t ep_num)
{
	fill_tx_fifo(usb_dc_get_ep_in_state(ep_num));
}
...
static void usb_dc_smartbond_isr(void)
{
	...
	if (GET_BIT(int_status, USB_USB_MAEV_REG_USB_FRAME)) {
		...
		} else if (usb_dc_state.nfsr == NFSR_NODE_WAKING) {
			/* No need to call set_nfsr, just set state */
			usb_dc_state.nfsr = NFSR_NODE_WAKING2;
		} else if (usb_dc_state.nfsr == NFSR_NODE_WAKING2) {
			/* No need to call set_nfsr, just set state */
			usb_dc_state.nfsr = NFSR_NODE_RESUME;
			LOG_DBG("usb_dc_state.nfsr = NFSR_NODE_RESUME %02x",
				USB->USB_MAMSK_REG);
		} else if (usb_dc_state.nfsr == NFSR_NODE_RESUME) {
			set_nfsr(NFSR_NODE_OPERATIONAL);
			if (usb_dc_state.ep_state[0][0].buffer != NULL) {
...
#if USE_SOF
			usb_dc_state.status_cb(USB_DC_SOF, NULL);
#else
			/*
...
{
	if (usb_dc_state.vbus_present == present) {
		return;
	}
	usb_dc_state.vbus_present = present;
...
static int usb_init(const struct device *arg)
{
	...
	for (int i = 0; i < 4; ++i) {
		usb_dc_state.ep_state[0][i].regs = reg_sets[i];
		usb_dc_state.ep_state[0][i].ep_addr = i | USB_EP_DIR_OUT;
		usb_dc_state.ep_state[0][i].buffer = ep_out_bufs[i];
		usb_dc_state.ep_state[1][i].regs = reg_sets[i];
		usb_dc_state.ep_state[1][i].ep_addr = i | USB_EP_DIR_IN;
	}
	/* Max packet size for EP0 is hardwired to 8 */
	usb_dc_state.ep_state[0][0].max_packet_size = EP0_FIFO_SIZE;
	usb_dc_state.ep_state[1][0].max_packet_size = EP0_FIFO_SIZE;

	z_smartbond_clock_pll_request();
...
{
	struct usb_ep_state *ep_state = usb_dc_get_ep_state(ep);
...
int usb_dc_ep_mps(const uint8_t ep)
{
	...
	return ep_state->max_packet_size;
}

int usb_dc_ep_read_continue(uint8_t ep)
{
	struct usb_ep_state *ep_state = usb_dc_get_ep_out_state(ep);
...
{
	uint8_t ep_num = USB_EP_GET_IDX(cfg->ep_addr);

	LOG_DBG("ep %x, mps %d, type %d", cfg->ep_addr, cfg->ep_mps, cfg->ep_type);

	if ((cfg->ep_type == USB_DC_EP_CONTROL && ep_num != 0) ||
	    (cfg->ep_type != USB_DC_EP_CONTROL && ep_num == 0)) {
...
int usb_dc_ep_clear_stall(const uint8_t ep)
{
	...
	struct usb_ep_state *ep_state = usb_dc_get_ep_state(ep);
	struct usb_ep_reg_set *regs = reg_sets[ep_num];
...
int usb_dc_ep_enable(const uint8_t ep)
{
	...
	if (ep_state->ep_addr == EP0_IN) {
		...
		USB->USB_RXMSK_REG |= 0x11 << (ep_num - 1);
		REG_SET_BIT(USB_MAMSK_REG, USB_M_RX_EV);
		ep_state->regs->epc_out |= USB_USB_EPC1_REG_USB_EP_EN_Msk;
		if (ep_state->transfer_in_progress) {
			return 0;
		}
		start_rx_packet(ep_state);
	} else {
		USB->USB_TXMSK_REG |= 0x11 << (ep_num - 1);
...
{
	struct usb_ep_state *ep_state = usb_dc_get_ep_state(ep_cfg->ep_addr);
	uint8_t ep_num = USB_EP_GET_IDX(ep_cfg->ep_addr);
	...
	if (ep_cfg->ep_type == USB_DC_EP_CONTROL) {
		ep_state->max_packet_size = EP0_FIFO_SIZE;
	} else {
		ep_state->max_packet_size = ep_cfg->ep_mps;
	}
	ep_state->data1 = 0;
	if (ep_dir == USB_EP_DIR_OUT) {
		if (ep_cfg->ep_mps > ep_out_buf_size[ep_num]) {
			return -EINVAL;
		}
		ep_state->regs->epc_out = ep_num | iso_mask;
	} else {
		ep_state->regs->epc_in = ep_num | iso_mask;
	}
		    ...
		    uint32_t *const ret_bytes)
{
	...
	if (!atomic_cas(&ep_state->transfer_in_progress, 0, 1)) {
