...
{
	struct rtc_smartbond_data *data = (struct rtc_smartbond_data *)dev->data;
...
#if defined(CONFIG_RTC_ALARM)
static uint32_t alarm_calendar_to_bcd(const struct rtc_time *timeptr)
{
	uint32_t rtc_calendar_alarm_reg = 0;

	RTC_CALENDAR_ALARM_REG_SET_FIELD(D, rtc_calendar_alarm_reg, bin2bcd(timeptr->tm_mday));
	RTC_CALENDAR_ALARM_REG_SET_FIELD(M, rtc_calendar_alarm_reg, bin2bcd(timeptr->tm_mon + 1));
...
{
	return rtc_time_to_bcd(timeptr);
}
						...
						const struct rtc_time *timeptr)
{
	...
	__unused uint32_t rtc_time_alarm_reg;
	__unused uint32_t rtc_calendar_alarm_reg;
	__unused uint32_t rtc_alarm_enable_reg;
	__unused uint32_t rtc_status_reg;

	if (id != RTC_ALARM_VALID_ID) {
	...
	if (mask) {
		...
		RTC->RTC_CALENDAR_ALARM_REG = alarm_calendar_to_bcd(timeptr);
