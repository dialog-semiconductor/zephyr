...
static inline uint8_t lcdc_smartbond_pixel_to_bytes(enum display_pixel_format pixfmt)
{
	switch (pixfmt) {
	case PIXEL_FORMAT_RGB_565:
		return 2;
					...
					enum display_pixel_format pixfmt)
{
	...
	if ((desc->width * desc->height * lcdc_smartbond_pixel_to_bytes(pixfmt)) !=
		desc->buf_size) {
		LOG_ERR("Buffer discriptor's porperties mismatch the requested number of bytes");
		return -EIO;
	}
