...
static void mipi_dbi_pm_get(const struct device *dev)
{
	 ...
	 * the extended sleep state. By calling this API, an internal counter is
	 ...
	 * system is idle).
     */
	(void)da1469x_pd_acquire(MCU_PD_DOMAIN_SYS);
#endif
#endif
}
...
#if CONFIG_PM
	(void)da1469x_pd_release(MCU_PD_DOMAIN_SYS);
#endif
#endif
}
...
static int mipi_dbi_smnartbond_reset(const struct device *dev, uint32_t delay)
{
	...
	if (config->reset.port == NULL) {
		LOG_WRN("Reset signal not present");
		return -ENOTSUP;
	}
...
/* Dispplay pixel to output color format translation */
					...
					enum display_pixel_format pixfmt)
{
	 ...
	 * quite slow). VSYNC interrupt line should be asserted when the last line is being outputed.
	 */
	if (da1469x_lcdc_is_busy()) {
		LOG_WRN("MIPI DBI host is busy");
		ret = -EBUSY;
...
static int mipi_dbi_smartbond_configure(const struct device *dev)
{
	 ...
	 */
	da1469x_lcdc_set_status(true, MIPI_DBI_SMARTBOND_IS_PLL_REQUIRED, (clk_div >= 2 ? clk_div / 2 : clk_div));
...
static int mipi_dbi_smartbond_resume(const struct device *dev)
{
	...
	if (!device_is_ready(clock_dev)) {
		LOG_WRN("Clock device is not available and so, no system clock update can be done.");
	} else {
...
static int mipi_dbi_smartbond_suspend(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_WRN("Could not apply MIPI DBI pins' sleep state. Consumption might be increased.");
	}
...
static int mipi_dbi_smartbond_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		ret = mipi_dbi_smartbond_suspend(dev);
	...
	case PM_DEVICE_ACTION_RESUME:
		 ...
		 * extended sleep state (Instead, use __WFI).
		 */
		if (ret == 0) {
